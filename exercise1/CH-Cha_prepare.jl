...
local_data_dir = joinpath(@__DIR__, "data")
shared_data_dir = local_data_dir

input_data = matread(joinpath(local_data_dir, "Data_CH-Cha_run.mat"))
ca_data = matread(joinpath(shared_data_dir, "Ca_Data.mat"))
ca_data["Date_CO2"] = vec(ca_data["Date_CO2"])

# Define the first and last iterations of the data to be used. This is only used to limit
# the number of timesteps for testing purposes and should be set to the full range
# when running a full simulation.
start_time = 1
end_time = 87648
...
Zs =
    FT.([0, 10, 20, 50, 100, 150, 200, 300, 400, 500, 600, 800, 1000, 1300])
...
YAML.write_file(joinpath(@__DIR__, "data", "CH-Cha.yaml"), data)
